...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_dcam_import_xxx
namespace = Collection("dcam_import")
namespace.configure(
    {
        "dcam_import": {
            "netbox_ver": "3.5.9",
            "python_ver": "3.10",
            "project_name": "dcam_imports",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.dcam_import.compose_http_timeout,
        "NETBOX_VER": context.dcam_import.netbox_ver,
        "PYTHON_VER": context.dcam_import.python_ver,
    }
    compose_command = f'docker-compose --project-name {context.dcam_import.project_name} --project-directory "{context.dcam_import.compose_dir}"'
    for compose_file in context.dcam_import.compose_files:
        compose_file_path = os.path.join(context.dcam_import.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.dcam_import.local):
        context.run(command, **kwargs)
    else:
        # Check if netbox is running, no need to start another netbox container to run a command
        docker_compose_status = "ps --services --filter status=running"
        results = docker_compose(context, docker_compose_status, hide="out")
        if "dcam-ui" in results.stdout:
            compose_command = f"exec dcam-ui {command}"
        else:
            compose_command = f"run --entrypoint '{command}' dcam-ui"
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Netbox with Python {context.dcam_import.python_ver}...")
